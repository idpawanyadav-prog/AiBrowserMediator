...
public sealed class CheckBoxXPathSetCheckedByClickWhenNeeded(IBrowserSession browserSession) : BrowserStrategyFunction(browserSession)
{
    public override string FunctionId => "CheckBox.XPath.SetCheckedByClickWhenNeeded";
    public override string StrategyName => "CheckBox.XPathStrategy";
    ...
    public override string Description => "Find checkbox by XPath and click only if current checked state differs from requested value.";
    ...
    public override async Task<CommandResult> ExecuteAsync(WorkflowStepDto step, CancellationToken cancellationToken = default)
    {
        ...
        return await BrowserSession.MatchesValueAsync(step.Locator, step.ControlType, step.Value, cancellationToken)
            ? Ok("Checkbox updated by XPath using click-when-needed.", step.Value)
